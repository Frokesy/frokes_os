...
export function ProfileModal({ initial, firstRun, onClose, onSaved }: { initial: UserProfile; firstRun: boolean; onClose: () => void; onSaved: (profile: UserProfile) => void }) {
  ...
  return <div role="dialog" aria-modal="true" aria-labelledby="profile-title" className="fixed inset-0 z-[70] flex items-end justify-center bg-black/70 backdrop-blur-md sm:items-center sm:p-6">
    <div className="relative flex max-h-dvh w-full flex-col overflow-hidden border-white/[.08] bg-[#0d1116] shadow-[0_35px_100px_rgba(0,0,0,.6)] sm:max-h-[min(760px,90dvh)] sm:max-w-[620px] sm:rounded-[28px] sm:border">
      <div className="h-1 bg-white/[.05]"><div className="h-full bg-[#b7f35b] transition-all duration-500" style={{ width: `${((step + 1) / 3) * 100}%` }}/></div>
      <header className="flex items-start justify-between px-6 pb-5 pt-6 sm:px-8 sm:pt-8"><div><p className="text-[10px] font-semibold uppercase tracking-[.22em] text-[#b7f35b]">{firstRun ? `Set up · ${copy.kicker}` : `Settings · ${copy.kicker}`}</p><h2 id="profile-title" className="mt-3 text-2xl font-medium tracking-tight sm:text-3xl">{copy.title}</h2><p className="mt-2 text-sm text-white/35">{copy.body}</p></div><button onClick={onClose} className="icon-button shrink-0" aria-label={firstRun ? "Finish later" : "Close settings"}><X size={17}/></button></header>
