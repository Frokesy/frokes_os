...
export function AppShell({ userName, userId, timeZone }: { userName: string; userId: string; timeZone: string }) {
  ...
  const beginToday = () => {
    ...
    timer.current = window.setTimeout(() => { setClockVisible(false); setTransition("idle"); }, 720);
  };
  ...
  return <div className="min-h-dvh bg-[#080a0d] text-[#f4f5ef]">
    <div className={shellMotion} aria-hidden={clockVisible && transition === "idle"}>
      ...
      <main className="pb-24 md:ml-[244px] md:pb-8">{view === "home" ? <TodayView store={store} userName={userName} timeZone={timeZone} onReturnToClock={returnToClock}/> : <HistoryView records={store.records} wordsOnly={view === "library"}/>}</main>
    ...
    {clockVisible && <div className={`fixed inset-0 z-50 bg-[#080a0d] ${transition === "to-home" ? "arrival-scene-exit pointer-events-none" : transition === "to-clock" ? "arrival-scene-enter" : ""}`}>
      <HomeView userName={userName} userId={userId} timeZone={timeZone} onProceed={beginToday}/>
    </div>}
  </div>;
}
