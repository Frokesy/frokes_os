...
export function TodayView({ store, userName, timeZone, onReturnToClock }: { store: Store; userName: string; timeZone: string; onReturnToClock: () => void }) {
  ...
  return <div className="mx-auto max-w-[1180px] px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
    <button onClick={onReturnToClock} className="mb-6 inline-flex items-center gap-2 rounded-lg border border-white/[.07] px-2.5 py-2 text-[10px] text-white/30 transition hover:bg-white/[.04] hover:text-white/65" aria-label="Return to clock"><Clock3 size={13}/> Clock</button>
...
function SyncIndicator({ status, pending, retry }: { status: SyncStatus; pending: number; retry: () => Promise<void> }) {
  if (status === "synced") return <span title="Your entries are saved to the cloud" className="flex items-center gap-1 text-[10px] text-white/25"><Cloud size={12}/> Synced</span>;
  if (status === "syncing" || status === "loading") return <span className="flex items-center gap-1 text-[10px] text-white/30"><RefreshCw size={12} className="animate-spin"/> {pending ? `Saving ${pending}` : "Syncing"}</span>;
  return <button onClick={() => void retry()} title="Your edits are safe on this device and will retry automatically" className="flex items-center gap-1 text-[10px] text-[#d6b878]"><CloudOff size={12}/> {status === "offline" ? `${pending || ""} saved offline` : "Retry sync"}</button>;
}
