...
export function MoodCard({ value, onSave }: { value?: MoodEntry; onSave: (mood: MoodEntry) => void }) {
  ...
  return <section className="card p-5 sm:p-7"><div className="mb-6 flex items-center justify-between"><div className="eyebrow"><Activity size={13}/> Check in</div>{value && <span className="flex items-center gap-1 text-xs text-[#b7f35b]"><Check size={13}/> Logged</span>}</div>
    ...
    <div className="mt-6 grid grid-cols-5 gap-2">{moods.map((item) => <button key={item.id} onClick={() => { setMood(item.id); setOpen(true); }} className={`rounded-xl border py-3 transition ${mood === item.id && (open || value?.mood === item.id) ? "border-[#b7f35b]/50 bg-[#b7f35b]/10" : "border-white/[.07] bg-white/[.025] hover:bg-white/[.05]"}`}><span className="block text-sm text-white/65">{item.face}</span><span className="mt-1 block text-[10px] text-white/35">{item.label}</span></button>)}</div>
